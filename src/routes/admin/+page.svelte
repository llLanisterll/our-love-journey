<script>
    ...
    let tabs = $derived([
        ...
        { id: 'gallery', name: 'Galeri', icon: '📸', badge: data.gallery ? data.gallery.length : 0 }
    ]);
</script>
    ...
    <main class="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-8 space-y-5">
        ...
        {#if activeTab === 'gallery'}
            <div transition:fade={{ duration: 200 }} class="space-y-6">
                <!-- Form Multi Upload -->
                <div class="bg-white border border-[#f4acb7]/30 rounded-3xl p-5 sm:p-8 shadow-xs space-y-5">
                    <h2 class="font-serif-title text-xl sm:text-2xl font-bold text-[#4a3b32]">Unggah Foto Galeri (Multi Upload)</h2>
                ...
                <!-- Grid Galeri Aktif -->
                <div class="bg-white border border-[#f4acb7]/30 rounded-3xl p-5 sm:p-8 shadow-xs space-y-5">
                    <h2 class="font-serif-title text-xl sm:text-2xl font-bold text-[#4a3b32]">Daftar Foto Galeri Aktif ({data.gallery.length})</h2>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3.5">
                            {#each data.gallery as photo (photo.id)}
                                <div class="relative group rounded-2xl overflow-hidden border border-[#f4acb7]/30 shadow-xs bg-[#fdf8f5]">
                                    <img src={photo.image_url} alt={photo.caption} class="w-full h-36 sm:h-40 object-cover group-hover:scale-105 transition-transform duration-300" />
                                    <div class="p-2 text-center bg-white/90">
                                        <p class="text-[11px] truncate font-semibold text-[#4a3b32]">{photo.caption || 'Foto'}</p>
                                    </div>
                                    <form method="POST" action="?/deleteGallery" use:enhance class="absolute top-2 right-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                                        <input type="hidden" name="id" value={photo.id} />
                                        <button 
                                            type="submit" 
                                            onclick={(e) => !confirm('Hapus foto ini dari galeri?') && e.preventDefault()}
                                            class="w-7 h-7 sm:w-8 sm:h-8 bg-rose-600 hover:bg-rose-700 text-white rounded-full flex items-center justify-center text-xs shadow-md active:scale-95"

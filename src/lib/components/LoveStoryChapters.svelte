...
<section id="chapters" class="py-20 px-4 max-w-5xl mx-auto text-center perspective-container">
    <div class="space-y-8">
        <!-- Title: Hidden initially, revealed on scroll -->
        <div use:scrollReveal={{ delay: 0, distance: 55 }} class="space-y-3 mb-12">
        ...
        <!-- Chapter Selector Pills: Hidden initially, revealed on scroll -->
        <div use:scrollReveal={{ delay: 180, distance: 45 }} class="flex overflow-x-auto justify-center space-x-3 pb-2">
        ...
        <!-- Active Chapter Content Card: Hidden initially, revealed on scroll with 3D tilt -->
        <div use:scrollReveal={{ delay: 300, distance: 65 }}>
            {#key activeChapter}
                {#if chapters[activeChapter]}
                    <div 
                        in:fly={{ y: 25, duration: 500 }}
                        use:tilt3d={{ maxTilt: 6, perspective: 1000 }}
                        class="max-w-3xl mx-auto bg-white/80 backdrop-blur-sm border border-[#f4acb7]/40 rounded-3xl p-8 md:p-12 shadow-sm hover:shadow-xl relative overflow-hidden space-y-6 text-center card-3d-hover"
                    >
                        <div class="w-16 h-16 rounded-full bg-[#fff0f3] border border-[#f4acb7]/40 flex items-center justify-center text-3xl mx-auto shadow-sm">
